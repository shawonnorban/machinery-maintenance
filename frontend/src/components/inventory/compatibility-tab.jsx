  ...
  function runDelete(id) {
    startTransition(async () => {
      ...
      if (result?.status === "success") {
        toastManager.add({ title: t("removed_toast"), type: "success" });
        router.refresh();
    ...
    <div className="flex flex-col gap-4">
      <Card className="divide-y divide-border">
        {compatibility.length === 0 ? (
          <p className="p-4 text-sm text-foreground-muted">{t("no_compatibility")}</p>
        ) : (
            ...
            <div key={row.id} className="flex items-center justify-between gap-3 p-4">
              <div className="flex items-center gap-2 text-sm">
                {row.compatibility_type === "FITS" ? (
                  <>
                    <Badge variant="info">{t("fits")}</Badge>
                    <span>{row.asset_model?.model}</span>
                  </>
                ) : (
                  <>
                    <Badge variant="neutral">{t("substitute")}</Badge>
                    <span>
              ...
              <Button variant="ghost" size="icon" aria-label={tc("delete")} loading={pending} onClick={() => runDelete(row.id)}>
  ...
  return (
    <Card className="p-4">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <FormField label={t("fits_machine_model")} required error={state?.errors?.asset_model_id?.[0]}>
          {(fieldProps) => (
            <Select
              {...fieldProps}
              value={assetModelId}
...
function SubstituteForm({ otherParts, action }) {
  ...
  return (
    <Card className="p-4">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <FormField label={t("substitutes_for")} required error={state?.errors?.substitute_for_part_id?.[0]}>
          {(fieldProps) => (
            <Select
              {...fieldProps}
