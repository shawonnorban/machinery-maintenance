...
function IssueReturnForm({ parts, bins, issueAction, returnAction }) {
  ...
  return (
    <Tabs value={mode} onValueChange={setMode}>
      <TabsList>
        <TabsTab value="issue">Issue</TabsTab>
        <TabsTab value="return">Return</TabsTab>
        <TabsIndicator />
      </TabsList>

      <TabsPanel value="issue">
        <MovementForm parts={parts} bins={bins} action={issueAction} verb="Issue" successLabel="Issued" />
      </TabsPanel>
      <TabsPanel value="return">
        <MovementForm parts={parts} bins={bins} action={returnAction} verb="Return" successLabel="Returned" />
      </TabsPanel>
    </Tabs>
  );
}
  ...
  return (
    <form action={dispatch} className="mt-4 flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FormField label="Spare part" required error={state?.errors?.spare_part_id?.[0]}>
          {(fieldProps) => (
            <Select
              {...fieldProps}
              name="spare_part_id"
      ...
      <FormField label="Notes" required helperText="Stock moving with no work order behind it is indistinguishable from loss without one." error={state?.errors?.notes?.[0]}>
