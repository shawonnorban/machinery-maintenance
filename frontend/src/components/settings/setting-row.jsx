...
function SettingRow({ definition, value, level, editableHere, factoryId, updateAction, resetAction }) {
  ...
  useEffect(() => {
    if (state?.status === "success") {
      toastManager.add({ title: `${definition.name} saved`, type: "success" });
  ...
  async function handleReset() {
    ...
    if (result?.status === "success") {
      toastManager.add({ title: `${definition.name} now follows the company`, type: "success" });
  ...
  return (
    <div className="flex flex-col gap-2.5 rounded-sm border-b border-border px-2 py-3 transition-colors last:border-b-0 hover:bg-surface-muted/60">
      <div className="flex items-start justify-between gap-3">
        ...
        <Badge variant={LEVEL_VARIANT[level] ?? "neutral"} className="shrink-0">{level}</Badge>
      </div>

      {editableHere ? (
        <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-2">
          <div className="w-full sm:w-64">
            <FormField label="Value" error={state?.errors?.value?.[0]}>
              {(fieldProps) => <FieldInput {...fieldProps} definition={definition} value={draft} onChange={setDraft} />}
            </FormField>
          </div>
        ...
        <p className="text-sm text-foreground-muted">
          {formatDisplay(definition.value_type, value)} — not editable at this level.
        </p>
      )}
    </div>
  );
...
function FieldInput({ definition, value, onChange, ...fieldProps }) {
  ...
  return (
    <Input
      ...
      placeholder={definition.value_type === "LIST" ? "Comma-separated" : undefined}
    />
  );
}
...
function formatDisplay(valueType, value) {
  if (valueType === "BOOL") return value ? "On" : "Off";
