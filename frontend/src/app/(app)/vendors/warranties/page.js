...
export default async function WarrantiesPage({ searchParams }) {
  ...
  const [warranties, expiringCount] = await Promise.all([
  ...
  return (
    <>
      <PageHeader
        breadcrumb={[{ label: "Vendors" }, { label: "Warranties" }]}
        title="Warranties"
        actions={
          <Link href="/vendors/warranties/create" className={cn(buttonVariants({ size: "sm" }))}>
            <Plus /> Record warranty
          </Link>
        }
      />

      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Expiring soon" value={expiringCount} icon={<AlertTriangle />} tone={expiringCount > 0 ? "warning" : "success"} />
      </div>
